...
bool		RectInRect(KRect& s1, KRect& s2)
{
	POINT cS1, cS2;
	cS1.x = (s1.w + s1.x) / 2;
	cS1.y = (s1.h + s1.y) / 2;
...
bool		SphereInSphere(KSphere& s1, KSphere& s2)
{
...
bool		SphereInSphere(RECT& s1, RECT& s2)
{
	POINT cS1, cS2;
	cS1.x = (s1.right + s1.left) / 2;
	cS1.y = (s1.bottom + s1.top) / 2;
	cS2.x = (s2.right + s2.left) / 2;
	cS2.y = (s2.bottom + s2.top) / 2;
